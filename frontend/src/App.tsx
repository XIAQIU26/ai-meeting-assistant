...
function AppContent() {
  ...
  const [guestMode, setGuestMode] = useState<boolean>(() => localStorage.getItem('arm_guest_mode') === '1');
  ...
  if (!user && !guestMode) {
    return <AuthPage onGuestMode={() => {
      localStorage.setItem('arm_guest_mode', '1');
      setGuestMode(true);
    }} />;
  }
  ...
  function handleLogout() {
    localStorage.removeItem('arm_guest_mode');
    setGuestMode(false);
  }
